...
int trackObject = 0;    //1表示由追踪对象，0表示无追踪对象 -1 表示追踪对象尚未计算Camshift 所需打属性

cv::Rect selection;     //保存鼠标选择的区域

cv::Mat image;          //用于缓存读取到的视频帧

//OpenCV 对所注册打鼠标毁掉函数定义为：
...
void onMouse( int event,int x,int y,int ,void* )
{
    ...
    switch(event){
        ...
        trackObject = -1; //追踪打目标还未计算Camshift所需打属性
        break;
    }
}
...
int main(int argc,const char** argv)
{
    ...
    cv::namedWindow("CamShift at yanxj");

    //1.注册鼠标事件的回调函数，第三个参数时用户提供给回调函数的，也就是回调函数中最后的param参数
    cv::setMouseCallback("CamShift at yanxj",onMouse,0);

    //捕获画面的容器，OpenCV中打Mat对象
    //OpenCV中最关键打Mat类，Mat是Matrix（矩阵）
    ...
    while(true){
        ...
        //将frame 中打图像写入全局变量image作为进行camshift的缓存
        frame.copyTo(image);
        ...
        if(trackObject)
        {
            ...
            //如果需要追踪的物体还没有进行属性提取，则对选择的目标中打图像属性提取
            if( trackObject < 0 ){
